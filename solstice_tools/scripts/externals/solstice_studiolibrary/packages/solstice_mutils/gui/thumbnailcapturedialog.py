...
print solstice_mutils.gui.capture("c:/temp/test.jpg", startFrame=1, endFrame=100)
...
import solstice_mutils.gui
import solstice_mutils.gui.modelpanelwidget
...
def tempThumbnailPath(clean=False):
    ...
    :rtype: str
    """
    tempDir = solstice_mutils.TempDir("icon", clean=clean)
...
def tempPlayblastPath(clean=False):
    ...
    :rtype: str
    """
    tempDir = solstice_mutils.TempDir("sequence", clean=clean)
    return tempDir.path() + "/thumbnail.jpg"
...
def thumbnailCapture(
    ...
    d = solstice_mutils.gui.ThumbnailCaptureDialog(
        path=path,
    ...
    if not show and not solstice_studioqt.isControlModifier():
        d.capture()
        d.close()
...
class ThumbnailCaptureDialog(QtWidgets.QDialog):
    ...
    def __init__(self, path="", parent=None, startFrame=None, endFrame=None, step=1):
        ...
        :type step: int
        """
        parent = parent or solstice_mutils.gui.mayaWindow()
        ...
        self._modelPanelWidget = solstice_mutils.gui.modelpanelwidget.ModelPanelWidget(self)
    ...
    def capture(self):
        ...
        self._capturedPath = solstice_mutils.playblast.playblast(
            path,
            modelPanel,
            startFrame,
            endFrame,
