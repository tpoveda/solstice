...
def saveAnim(
    ...
    :rtype: solstice_mutils.Animation
    """
    step = 1
    ...
    if not time:
        time = solstice_mutils.selectedObjectsFrameRange(objects)
    ...
    sequencePath = solstice_mutils.createSnapshot(
        path=sequencePath,
        start=start,
        end=end,
        step=step,
    ...
    anim = solstice_mutils.Animation.fromObjects(objects)
...
def loadAnims(
    ...
    for path in paths:
        ...
        anim = solstice_mutils.Animation.fromPath(path)
...
class Animation(solstice_mutils.Pose):
    ...
    def __init__(self):
        solstice_mutils.Pose.__init__(self)
    ...
    def read(self, path=None):
        ...
        solstice_mutils.Pose.read(self, path=path)
        logger.debug("Reading Done")

    @solstice_mutils.unifyUndo
    @solstice_mutils.restoreSelection
    def open(self):
        """
    ...
    @solstice_mutils.timing
    @solstice_mutils.unifyUndo
    @solstice_mutils.showWaitCursor
    @solstice_mutils.restoreSelection
    def save(
        self,
        path,
        ...
        if solstice_mutils.getDurationFromNodes(nodes=objects or []) <= 0:
        ...
        try:
            if bakeConnected:
                ...
                solstice_mutils.bakeConnected(objects, time=(start, end), sampleBy=sampleBy)
            ...
            for name in objects:
                if maya.cmds.copyKey(name, time=(start, end), includeUpperBound=False, option="keys"):
                    ...
                    solstice_mutils.disconnectAll(transform)
                    ...
                    for attr in attrs:
                        ...
                        if dstCurve:
                            ...
                            srcCurve = solstice_mutils.animCurve("%s.%s" % (name, attr))
            ...
            solstice_mutils.Pose.save(self, posePath)
    ...
    def load(
        ...
        :type option: PasteOption
        :type connect: bool
        :type mirrorTable: solstice_mutils.MirrorTable
        ...
        valid = False
        matches = list(solstice_mutils.matchNames(srcObjects=srcObjects, dstObjects=objects, dstNamespaces=namespaces))
        ...
        if not matches or not valid:
            ...
            raise solstice_mutils.NoMatchFoundError(text)
        ...
        try:
            ...
            for srcNode, dstNode in matches:
                ...
                for attr in self.attrs(srcNode.name()):
                    ...
                    dstAttr = solstice_mutils.Attribute(dstNode.name(), attr)
