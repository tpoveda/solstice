...
class CombinedWidgetItem(QtWidgets.QTreeWidgetItem):
    ...
    def setIcon(self, column, icon, color=None):
        ...
        if isinstance(icon, basestring):
            if not os.path.exists(icon):
                color = color or solstice_studioqt.Color(255, 255, 255, 20)
                icon = solstice_studioqt.resource.icon("image", color=color)
            else:
    ...
    def thumbnailIcon(self):
        ...
        if not os.path.exists(thumbnailPath):
            ...
            thumbnailPath = solstice_studioqt.resource.icon("thumbnail", color=color)
    ...
    def imageSequence(self):
        """
        :rtype: solstice_studioqt.ImageSequence
        """
        return self._imageSequence

    def setImageSequence(self, value):
        """
        :type value: solstice_studioqt.ImageSequence
        """
    ...
    def play(self):
        ...
        if os.path.isfile(path) and path.lower().endswith(".gif"):
            ...
            if not self.imageSequence():
                movie = solstice_studioqt.ImageSequence(path)
                movie.frameChanged.connect(self._frameChanged)
