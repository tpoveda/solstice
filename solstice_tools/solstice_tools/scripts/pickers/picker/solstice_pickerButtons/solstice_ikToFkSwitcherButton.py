...
""" ==================================================================
Script Name: solstice_ikToFkSwitcherButton.py
by Tomás Poveda
Custom button used by the picker of Solstice Short Film
______________________________________________________________________
Base class to be used in the ikFkSwitcher buttons
______________________________________________________________________
==================================================================="""
...
class solstice_ikToFkSwitcherButton(baseBtn.solstice_pickerBaseButton, object):
    def __init__(self,
                 x=0, y=0, text='', cornerRadius=5, width=30, height=15, btnInfo=None, parent=None):
        super(solstice_ikToFkSwitcherButton, self).__init__(
            x=x,
            y=y,
            text=text,
            radius=cornerRadius,
    ...
    def setInfo(self, btnInfo):
        super(solstice_ikToFkSwitcherButton, self).setInfo(btnInfo)
    ...
    def mousePressEvent(self, event):
        super(solstice_ikToFkSwitcherButton, self).mousePressEvent(event)
        self.getPart().setFK()
        sel = cmds.ls(sl=True)
        if len(sel) > 0:
            currCtrl = sel[0]
