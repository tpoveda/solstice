...
class solstice_keyFkIkButton(baseBtn.solstice_pickerBaseButton, object):
    def __init__(self,
                 x=0, y=0, text='', cornerRadius=5, width=30, height=15, btnInfo=None, parent=None):
        super(solstice_keyFkIkButton, self).__init__(
            x=x,
            y=y,
            text=text,
            radius=cornerRadius,
    ...
    def setInfo(self, btnInfo):
        super(solstice_keyFkIkButton, self).setInfo(btnInfo)
    ...
    def mousePressEvent(self, event):
        super(solstice_keyFkIkButton, self).mousePressEvent(event)
