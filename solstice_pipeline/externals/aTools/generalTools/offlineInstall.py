...
def aToolsOfflineInstall(offlineFilePath):
    ...
    zfobj.close()
    if os.path.isfile(tmpZipFile):     os.remove(tmpZipFile)
    from solstice_pipeline.externals.aTools import setup; reload(setup); setup.install([offlineFilePath, False])
    cmds.evalDeferred(\"from solstice_pipeline.externals.aTools.animTools.animBar import animBarUI; reload(animBarUI); animBarUI.show(\'refresh\')\")
