...
SETTINGS_PATH = os.path.join(HOME_PATH, 'solstice_studioqt.ini')
...
def createMessageBox(
    ...
    if headerIcon:
        p = solstice_studioqt.resource.pixmap(headerIcon)
        mb.setPixmap(p)
...
def showMessageBox(
    ...
    if solstice_studioqt.isControlModifier() or solstice_studioqt.isAltModifier():
        force = True
...
class MessageBox(QtWidgets.QDialog):
    ...
    def fadeIn(self, duration=200):
        ...
        """
        if self._frame:
            self._animation = solstice_studioqt.fadeIn(self._frame, duration=duration)
    ...
    def fadeOut(self, duration=200):
        ...
        """
        if self._frame:
            self._animation = solstice_studioqt.fadeOut(self._frame, duration=duration)
        return self._animation
