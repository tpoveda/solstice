...
mt = solstice_mutils.MirrorTable.fromObjects(objects, "_l_", "_r_", MirrorPlane.YZ)
...
path = "/tmp/mirrortable.json"
mt = solstice_mutils.MirrorTable.fromPath(path)
...
mt.load(option=solstice_mutils.MirrorOption.LeftToRight)

# Example 10:
# Mirror all objects from right to left
mt.load(option=solstice_mutils.MirrorOption.RightToLeft)
...
class MirrorTable(solstice_mutils.SelectionSet):
    ...
    def _transferAnimation(self, srcObj, dstObj, attrs=None, mirrorAxis=None, time=None):
        ...
        for attr in attrs:
            srcAttribute = solstice_mutils.Attribute(srcObj, attr)
            dstAttribute = solstice_mutils.Attribute(dstObj, attr)
