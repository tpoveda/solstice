...
from solstice_studiolibrarymaya import baseitem
from solstice_studiolibrarymaya import basecreatewidget
from solstice_studiolibrarymaya import basepreviewwidget

try:
    import solstice_mutils
    import solstice_mutils.gui
    import maya.cmds
    PasteOption = solstice_mutils.PasteOption
...
class AnimItem(baseitem.BaseItem):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setTransferClass(solstice_mutils.Animation)
    ...
    def previewWidget(self, libraryWidget):
        ...
        :type libraryWidget: solstice_studiolibrary.LibraryWidget
    ...
    def save(
        ...
        tempDir = solstice_mutils.TempDir("Transfer", clean=True)
        ...
        solstice_studiolibrary.LibraryItem.save(self, path=path, contents=contents)
...
class AnimCreateWidget(basecreatewidget.BaseCreateWidget):
    ...
    def __init__(self, item=None, parent=None):
        ...
        try:
            start, end = solstice_mutils.currentFrameRange()
        except NameError, e:
            logger.exception(e)
        ...
        settings = solstice_studiolibrarymaya.settings()
    ...
    def createSequenceWidget(self):
        ...
        :rtype: None
        """
        self.ui.sequenceWidget = solstice_studioqt.ImageSequenceWidget(self)
        ...
        icon = solstice_studiolibrarymaya.resource().icon("thumbnail2")
    ...
    def showFrameRangeMenu(self):
        ...
        :rtype: None
        """
        action = solstice_mutils.gui.showFrameRangeMenu()
        if action:
    ...
    def showByFrameDialog(self):
        ...
        if self.duration() > 100 and self.byFrame() == 1:
            ...
            result = solstice_studioqt.MessageBox.question(
                self.libraryWidget(),
                title="Anim Item Tip",
                text=text,
                buttons=buttons,
    ...
    def _thumbnailCaptured(self, playblastPath):
        ...
        :rtype: None
        """
        thumbnailPath = solstice_mutils.gui.tempThumbnailPath()
    ...
    def thumbnailCapture(self):
        ...
        :raise: AnimItemError
        """
        startFrame, endFrame = solstice_mutils.selectedFrameRange()
        ...
        if not solstice_studioqt.isControlModifier():
            self.showByFrameDialog()

        try:
            step = self.byFrame()
            playblastPath = solstice_mutils.gui.tempPlayblastPath()

            solstice_mutils.gui.thumbnailCapture(
                path=playblastPath,
                startFrame=startFrame,
                endFrame=endFrame,
                step=step,
...
class AnimPreviewWidget(basepreviewwidget.BasePreviewWidget):
    ...
    def optionChanged(self, text, save=True):
        ...
        imageIcon = solstice_studiolibrarymaya.resource().icon(basename)
...
iconPath = solstice_studiolibrarymaya.resource().get("icons", "animation.png")
...
solstice_studiolibrary.registerItem(AnimItem)
