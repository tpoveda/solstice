...
class MirrorItem(baseitem.BaseItem):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setTransferClass(solstice_mutils.MirrorTable)
    ...
    def save(
        ...
        tempDir = solstice_mutils.TempDir("Transfer", makedirs=True)
        ...
        solstice_studiolibrary.LibraryItem.save(
            self,
            path=path,
...
class MirrorCreateWidget(basecreatewidget.BaseCreateWidget):
    ...
    def selectionChanged(self):
        ...
        if not self.ui.left.text():
            self.ui.left.setText(solstice_mutils.MirrorTable.findLeftSide(objects))

        if not self.ui.right.text():
            self.ui.right.setText(solstice_mutils.MirrorTable.findRightSide(objects))

        leftSide = str(self.ui.left.text())
        rightSide = str(self.ui.right.text())

        mt = solstice_mutils.MirrorTable.fromObjects(
                [],
                leftSide=leftSide,
                rightSide=rightSide
        )
...
iconPath = solstice_studiolibrarymaya.resource().get("icons", "mirrorTable.png")
...
solstice_studiolibrary.registerItem(MirrorItem)
