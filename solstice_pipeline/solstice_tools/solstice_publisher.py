...
class PublishShadingTask(solstice_task.Task, object):
    ...
    def run(self):
        ...
        # Check if shading file exists
        shading_file = os.path.join(self._asset().asset_path, '__working__', 'shading', self._asset().name + '_SHD.ma')
        if not os.path.exists(shading_file):
            self.write_error('Shading file {} does not exists!'.format(shading_file))
            return False

        # Create backup of the file
        self.write('Creating Shading backup file')
        filename, extension = os.path.splitext(shading_file)
        backup_file = filename + '_BACKUP' + extension
        copyfile(shading_file, backup_file)
        ...
        try:
            ...
            self.write('Writing new textures paths info into shading file: {}'.format(shading_file))
            with open(shading_file, 'w') as f:
            ...
            new_size = python.get_size(shading_file)
            ...
            if diff_size > 1000000:
                ...
                try:
                    os.remove(shading_file)
                    os.rename(backup_file, shading_file)
                except Exception as e:
            ...
            artella.open_file_in_maya(file_path=model_path)
