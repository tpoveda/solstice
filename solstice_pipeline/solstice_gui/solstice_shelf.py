...
class SolsticeShelf(object):
    ...
    def create(self, delete_if_exists=True):
        ...
        if delete_if_exists:
            if sp.is_houdini():
                ...
                if solstice_houdini_utils.shelf_set_exists(shelf_set_name=self.name):
                    solstice_houdini_utils.remove_shelf_set(name=self.name)
            else:
            ...
            if sp.is_houdini():
                from solstice_pipeline.solstice_utils import solstice_houdini_utils
                assert not solstice_houdini_utils.shelf_set_exists(shelf_set_name=self.name), 'Shelf Set with name {} already exists!'.format(self.name)
            else:
    ...
    def add_button(self, label, tooltip=None, icon='customIcon.png', command=None, double_command=None, command_type='python'):
        ...
        if sp.is_houdini():
            from solstice_pipeline.solstice_utils import solstice_houdini_utils
            return solstice_houdini_utils.create_shelf_tool(
    ...
    def build_categories(self, shelf_file, categories):
        ...
        if sp.is_houdini():
            ...
            for cat in categories:
                ...
                if solstice_houdini_utils.shelf_exists(shelf_name=shelf_name):
                    solstice_houdini_utils.remove_shelf(name=shelf_name)
                if not solstice_houdini_utils.shelf_exists(shelf_name=shelf_name):
                    new_shelve = solstice_houdini_utils.create_shelf(shelf_name=shelf_name, shelf_label=cat.title())
                    all_shelves.append(new_shelve)
            shelf_set = solstice_houdini_utils.get_shelf_set(shelf_set_name=self.name)
        ...
        with open(shelf_file) as f:
            ...
            if sp.is_houdini():
                ...
                for item, item_data in shelf_data['solstice_shelf'].items():
                    ...
                    current_shelf = solstice_houdini_utils.get_shelf(shelf_name='solstice_{}'.format(item))
