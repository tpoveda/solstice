...
class PublishTexturesTask(task.Task, object):
    ...
    def run(self):
        ...
        check = validators.TexturesValidator(asset=self._asset)
        check.check()
        ...
        try:
            result = qtutils.show_question(None, 'Publishing textures {0}'.format(textures_path), 'Textures validated successfully! Do you want to continue with the publish process?')
            ...
            if result == QMessageBox.Yes:
                ...
                for txt in textures:
                    if valid_new_versions:
                        ...
                        if not valid_new_version:
                            ...
                            syncdialog.SolsticeSyncFile(files=[txt]).sync()
                ...
                if not os.path.exists(new_sync_file):
                    syncdialog.SolsticeSyncFile(files=[new_sync_file]).sync()
                if not os.path.exists(new_sync_file):
...
class PublishModelTask(task.Task, object):
    ...
    def run(self):
        ...
        check = validators.ModelValidator(asset=self._asset)
        check.check()
        if not check.is_valid():
        ...
        if result == QMessageBox.Yes:
            ...
            if not valid_new_version:
                ...
                artella.unlock_file(model_path)
                return False
            syncdialog.SolsticeSyncFile(files=[model_path]).sync()
...
class PublishShadingTask(task.Task, object):
    ...
    def run(self):
        ...
        # Do initial validation
        check = validators.ShadingValidator(asset=self._asset)
        check.check()
        if not check.is_valid():
            return False
        ...
        try:
            ...
            info = shaderlibrary.ShaderLibrary.export_asset(asset=self._asset(), shading_meshes=shading_meshes)
        ...
        result = qtutils.show_question(None, 'Publishing file {}'.format(shading_path), 'File validated successfully! Do you want to continue with the publish process?')
        published_done = False
        if result == QMessageBox.Yes:
            ...
            if not valid_new_version:
                ...
                artella.unlock_file(shading_path)
                return False
            syncdialog.SolsticeSyncFile(files=[shading_path]).sync()
...
class PublishGroomTask(task.Task, object):
...
class PublishTaskGroup(taskgroups.TaskGroup, object):
...
class SolsticePublisher(dialog.Dialog, object):
...
class AssetPublisherVersionWidget(QWidget, object):
    ...
    def __init__(self, asset, new_working_version=False, parent=None):
        ...
        self._asset_icon.setPixmap(
            resource.pixmap('empty_file', category='icons').scaled(200, 200, Qt.KeepAspectRatio))
        ...
        for i, category in enumerate(sp.valid_categories):
            ...
            self._ui[category]['separator'].setPixmap(
                resource.pixmap('arrow_material', category='icons').scaled(QSize(24, 24)))
        ...
        main_layout.addWidget(splitters.Splitter('Comment'))
    ...
    def _publish(self):
        try:
            ...
            if categories_to_publish['textures']['check']:
                result = qtutils.show_question(None, 'Publishing Files', 'Are you sure you want to publish textures? All of them will be versioned and this can take a lof of memory in Artella server. To publish single textures files do it directly thorugh Artella')
                if result == QMessageBox.No:
                    return
...
class AssetPublisherSummaryWidget(QWidget, object):
    ...
    def __init__(self, asset, parent=None):
        ...
        self._error_pixmap = resource.pixmap('error', category='icons')
        self._ok_pixmap = resource.pixmap('ok', category='icons')
        ...
        main_layout.addLayout(splitters.SplitterLayout())
        self._spinner = spinner.WaitSpinner()
        main_layout.addWidget(self._spinner)

        self.ok_widget = QWidget()
        ok_splitter = splitters.SplitterLayout()
