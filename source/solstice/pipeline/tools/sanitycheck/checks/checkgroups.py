...
class GeneralSanityCheck(SanityCheckGroup, object):
    def __init__(self, auto_fix=False, stop_on_error=False, parent=None):
        ...
        self.add_check(checks.CleanUnknownNodesCheck())
        self.add_check(checks.CleanOldPluginsCheck())
        self.add_check(checks.StudentLicenseCheck())
...
class AssetShadingPublishSantiyCheck(SanityCheckGroup, object):
    def __init__(self, asset, file_type, auto_fix=False, stop_on_error=False, parent=None):
        ...
        self.add_check(assetchecks.AssetFileExists(asset=asset, status='working', file_type=file_type))
        self.add_check(assetchecks.StudentLicenseCheck(asset=asset, status='working', file_type=file_type))
        self.add_check(assetchecks.ValidPublishedTextures(asset=asset, auto_fix=self.auto_fix))
        self.add_check(
            assetchecks.AssetFileSync(asset=asset, status='working', file_type=file_type, auto_fix=self.auto_fix))
        self.add_check(
            assetchecks.NotLockedAsset(asset=asset, status='working', file_type=file_type, auto_fix=self.auto_fix))
        self.add_check(assetchecks.TexturesFolderSync(asset=asset, auto_fix=self.auto_fix))
