...
class HighProxyEditor(taggereditor.TaggerEditor, object):
    ...
    def update_proxy_group(tag_data=None):
        ...
        if not tag_data:
            ...
            if not tp.Dcc.object_exists(tag_data):
                artellapipe.logger.error('Tag Data {} does not exists in current scene!'.format(tag_data))
                return False
        ...
        if len(sel_name) > 1:
            artellapipe.logger.error('Multiple assets with same name: {}'.format(current_selection))
            return False
        sel_name = sel_name[0]
        ...
        if tp.Dcc.object_exists(sel_name):
            ...
            if not children:
                artellapipe.logger.error('Proxy Group not found!')
                return False
            for obj in children:
                ...
                if base_name == proxy_name:
                    if proxy_path is None:
                        ...
                        artellapipe.logger.error('Multiple proxy groups in the asset. Asset only can have one proxy group: {}'.format(proxy_name))
                        return False
    ...
    def update_hires_group(tag_data=None):
        ...
        if tp.Dcc.object_exists(sel_name):
            ...
            children = tp.Dcc.list_relatives(node=sel_name, all_hierarchy=True, full_path=True, relative_type='transform')
            if not children:
                artellapipe.logger.error('Hires Group not found!')
                return False
            for obj in children:
                base_name = obj.split('|')[-1]
