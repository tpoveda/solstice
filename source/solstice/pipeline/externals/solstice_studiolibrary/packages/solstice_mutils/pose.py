...
p = solstice_mutils.Pose.fromObjects(objects)
...
# Load from file
path = "/tmp/pose.json"
p = solstice_mutils.Pose.fromPath(path)
...
def savePose(path, objects=None, iconPath=None, metadata=None):
    ...
    if iconPath:
        ...
        solstice_mutils.createSnapshot(iconPath_)

    pose = solstice_mutils.Pose.fromObjects(objects)
...
class Pose(solstice_mutils.SelectionSet):

    def __init__(self):
        """
        """
        solstice_mutils.SelectionSet.__init__(self)
        self._cache = None
        self._cacheKey = None
    ...
    def createObjectData(self, name):
        ...
        attrs = [solstice_mutils.Attribute(name, attr) for attr in attrs]
    ...
    def updateCache(self, objects=None, namespaces=None, dstAttrs=None, ignoreConnected=False,
        ...
        if self._cacheKey != cacheKey or not cache:
            ...
            matches = solstice_mutils.matchNames(
                srcObjects,
    ...
    def mirrorTable(self):
        """
        :rtype: solstice_mutils.MirrorTable
        """
    ...
    def updateMirrorAxis(self, srcNode, dstNode, mirrorAxis):
        """
        :type srcNode: solstice_mutils.Node
        :type dstNode: solstice_mutils.Node
        :type mirrorAxis: list[int]
        """
    ...
    def cacheNode(self, srcNode, dstNode, dstAttrs=None, ignoreConnected=None, onlyConnected=None, usingNamespaces=None):
        ...
        if usingNamespaces:
            ...
            except solstice_mutils.NoObjectFoundError, msg:
                logger.debug(msg)
                return
            except solstice_mutils.MoreThanOneObjectFoundError, msg:
                logger.debug(msg)
                return
        ...
        for attr in self.attrs(srcNode.name()):
            ...
            dstAttribute = solstice_mutils.Attribute(dstNode.name(), attr)
            ...
            srcAttribute = solstice_mutils.Attribute(dstNode.name(), attr, value=value, type=type_)
            dstAttribute.update()
