...
def ls(*args, **kwargs):
    ...
    :rtype: list[Node]
    """
    return [solstice_mutils.Node(name) for name in maya.cmds.ls(*args, **kwargs) or []]


def listAttr(node, **kwargs):
    """
    :type node: solstice_mutils.Node
    :type kwargs: dict
    :rtype: list[solstice_mutils.Attribute]
    """
    attrs = maya.cmds.listAttr(node.name(), **kwargs)
    return [solstice_mutils.Attribute(node.name(), attr) for attr in attrs or []]
...
def getSelectedObjects():
    """
    :rtype: list[str]
    :raise solstice_mutils.SelectionError:
    """
    selection = maya.cmds.ls(selection=True)
    if not selection:
        raise solstice_mutils.SelectionError("No objects selected!")
    return selection
...
def animCurve(fullname):
    ...
    :rtype: None | str
    """
    attribute = solstice_mutils.Attribute(fullname)
