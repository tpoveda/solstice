...
from solstice_studiolibrarymaya.main import main
...
SETTINGS_PATH = solstice_studiolibrary.localPath("LibraryItem.json")
...
def readSettings():
    ...
    :rtype: dict
    """
    return solstice_studiolibrary.readJson(SETTINGS_PATH)
...
def saveSettings(data):
    ...
    solstice_studiolibrary.updateJson(SETTINGS_PATH, data)
...
def settings():
    ...
    # Mirror options
    _settings.setdefault("mirrorOption", solstice_mutils.MirrorOption.Swap)
...
def resource():
    ...
    if not _resource:
        _resource = solstice_studioqt.Resource(RESOURCE_DIRNAME)
...
def enableMayaClosedEvent():
    ...
    if not _mayaCloseScriptJob:
        ...
        try:
            _mayaCloseScriptJob = solstice_mutils.ScriptJob(event=event)
        except NameError, e:
            logging.exception(e)
...
def mayaClosedEvent():
    ...
    :rtype: None
    """
    for libraryWidget in solstice_studiolibrary.LibraryWidget.instances():
...
def setDebugMode(libraryWidget, value):
    ...
    logger_ = logging.getLogger("solstice_mutils")
    logger_.setLevel(level)

    logger_ = logging.getLogger("solstice_studiolibrarymaya")
    logger_.setLevel(level)


solstice_studiolibrary.LibraryWidget.globalSignal.debugModeChanged.connect(setDebugMode)
